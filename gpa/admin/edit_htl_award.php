...
if(isset($_REQUEST[upload_htl_award])){
	...
	if($_FILES['doc_file']['name']!=''){
			...
			if( $_FILES['doc_file']['size'] > $max_file_size ){
				...
				$htl_award_save = mysql_query("UPDATE  hotel_awards SET  `employee_name` =  '$employee_name',
																		 `mqy_type` =  '$mqy_type',
																		 `award_my` =  '$award_my',
																		 `designation` =  '$designation',
																		 `departmentid` =  '$departmentid',
																		 `type_of_award` =  '$type_of_award',
																		 `employee_pic` =  '$profile_pic'
																		  WHERE  id = '$htla_id';");
          ...
          <table width="100%" border="0" cellspacing="8" cellpadding="0">        
            ...
            <tr>
              ...
              <h2>Add New Hotel Awards</h2>
                <hr />
                <?php
				$sel_htl_award = mysql_query("SELECT * FROM hotel_awards WHERE id = '$htla_id'");
				$sel_htl_award_res = mysql_fetch_array($sel_htl_award);
                ...
                <table width="100%" border="0" cellspacing="5" cellpadding="0">
                  <tr>
                    ...
                    <td> 
                      <select id="thechoices" name="mqy_type" class="adm_select">
                        <option value="yearly" <?php echo "$sel_htl_award_res[mqy_type]" == "yearly" ? "selected" : null?>>Yearly Award</option>
                        <option value="quarterly" <?php echo "$sel_htl_award_res[mqy_type]" == "quarterly" ? "selected" : null?> style="width:260px;">Quarterly  Award</option>
                        <option value="monthly" <?php echo "$sel_htl_award_res[mqy_type]" == "monthly" ? "selected" : null?> style="width:260px;">Monthly Award</option>
                      </select>
                    </td>
                  </tr>
                  <tr>
                    ...
                    <td>
                        <div id="boxes">
							...
							$m = date('n',strtotime($sel_htl_award_res[award_my]));
							$y = date('Y',strtotime($sel_htl_award_res[award_my]));
                  ...
                  <tr>
                    <td width="20%">Employee Name</td><td><input type="text" name="employee_name" id="first_name" class="adm_inptext" value="<?=$sel_htl_award_res[employee_name];?>"  /></td>
                  </tr>
                  <tr>
                    <td width="20%">Designation</td><td>
                      <input type="text" name="designation" id="designation" class="adm_inptext" value="<?=$sel_htl_award_res[designation];?>"  />
                    </td>
                  </tr>
                  <tr>
					...
					?>  
                    <option value="<?=$deptres[department_name]?>" <?php echo "$deptres[department_name]" == "$sel_htl_award_res[departmentid]" ? 'selected' : null?>>
                  ...
                  <tr>
                    <td width="20%">Type of Award</td><td>
                      <input type="text" name="type_of_award" id="type_of_award" class="adm_inptext" value="<?=$sel_htl_award_res[type_of_award];?>" />
                    </td>
                  </tr>
                  <tr>
                    <td>Picture</td><td> <img src="<?=$sel_htl_award_res[employee_pic];?>" width="140" /><br /><input name="doc_file" type="file" />
