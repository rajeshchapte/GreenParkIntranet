...
function get_survey($sid){
	$get_survey = mysql_query("SELECT * FROM survey_poll WHERE id = '$sid'");
	$row = mysql_fetch_array($get_survey);
	return $row[survey_name];
}

function get_survey_cats_count($sid){
	$get_survey_cats_count = mysql_query("SELECT COUNT(*) FROM survey_cats WHERE survey_id = '$sid'");
	$row = mysql_fetch_array($get_survey_cats_count);
	return $row['COUNT(*)'];;
}
