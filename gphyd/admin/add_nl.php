...
$nl =  mysql_query("SELECT MAX(id) as idm FROM news_letter WHERE prp_code = $prp_id");
$nl_res = mysql_fetch_array($nl);
$maxid = $nl_res[idm]+1;
...
if(isset($_REQUEST[upload_nl])){
	...
	$max_file_size = 1024 * 20480;
	$temp = explode(".", $_FILES["nl_file"]["name"]);
	$extension = end($temp);
	$allowedExts = array("pdf", "gif", "jpeg", "jpg", "png"); 
				if( ($_FILES['nl_file']['size'] < $max_file_size) && in_array($extension, $allowedExts) ){
						...
						$rfile="nl_file_".$nl_headline.$nl_date.$timestamp; //To get the name of the image
						...
						$vac_save = mysql_query("INSERT INTO `news_letter` (`id`, `news_title`, `news_desc`, `news_date`, `nl_file_path`, `prp_code`, `created_date`) VALUES (NULL, '$nl_headline', '$nl_desc', '$nl_date', '$profile_pic', '$prp_id', '$date');");
			...
			$vac_save = mysql_query("INSERT INTO `news_letter` (`id`, `news_title`, `news_desc`, `news_date`, `nl_file_path`, `prp_code`, `created_date`) VALUES (NULL, '$nl_headline', '$nl_desc', '$nl_date', NULL, '$prp_id', '$date');");
                ...
                <table width="100%" border="0" cellspacing="5" cellpadding="0">
                  <tr>
                    <td width="20%">Newsletter Date</td><td>
                      <input type="text" name="nl_date" id="nl_date" class="adm_inptext"  /><br />
                      <?php //if($error_msg != ''){
                  ...
                  <tr>
                    <td width="20%">Headline</td><td><input type="text" name="nl_headline" id="nl_headline" class="adm_inptext"  /></td>
                  </tr>
                  <tr>
                    <td width="20%">News Description</td><td>
                      <input type="text" name="nl_desc" id="nl_desc" class="adm_inptext"  />
                    </td>
                  </tr>
                  <tr>
                    <td width="20%">Newsletter File</td><td>
                      <input type="file" name="nl_file" id="nl_file"  /><br />
