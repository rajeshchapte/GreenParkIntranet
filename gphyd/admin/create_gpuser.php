...
if(isset($_REQUEST[upload_emp])){
		...
		$sqlLogin = "SELECT * from gp_user WHERE email_id='".mysql_real_escape_string(strtolower($_POST['email_id']))."' AND property_code = '$prp_id'";
		...
		if (mysql_num_rows($resultLogin) == 0 )
		{
			...
			$gender = $_POST[gender];
				...
				$inserUser = mysql_query("INSERT INTO gp_user (`id`, `first_name`, `last_name`, `email_id`, `password`, `location`, `dob`, `gender`, `property`, `property_code`, `status`, `role`, `created_datetime`) VALUES (NULL, '$first_name', '$last_name', '$email_id', '$password', '$res_prp[location]', '$birth_date', '$gender', '$res_prp[property_name]', '$prp_id', '1', 'user', '$date');");
