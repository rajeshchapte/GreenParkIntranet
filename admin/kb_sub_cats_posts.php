...
if(isset($_REQUEST[delete_kb])){
	$kb_id = $_POST[kb_id];
	$delsopfile = mysql_query("DELETE FROM knwledge_bank WHERE id = $kb_id");
	if($delsopfile){
		$delmsg = "Knwledge Bank Record Deleted";
		//unlink($somefile);
	}else{
		$delmsg =  "failed. ".mysql_error();;
	}
}
              ...
              <h2>Knowledge Bank</h2>
                ...
                <table width="100%" border="0" cellspacing="1" cellpadding="2" bgcolor="#CCCCCC">
				  ...
				  if($dept_sub_id=='NULL' || $dept_sub_id==''){
					...
					while($select_kb_res = mysql_fetch_array($select_kb)){
					?>	
                  <tr bgcolor="#ececef">
                    <td width="5%"><?=$i;?></td>
                    <td width="80%" style="text-transform:capitalize"><a href="<?=$admin_home;?>/kb_sub_cats_posts_desc.php?dept_id=<?=$dept_id;?>&dept_sub_id=<?=$dept_sub_id;?>&dept_gdc=GuestDelightActivities&kb_id=<?=$select_kb_res[id];?>" style="cursor:pointer;"><?=ucfirst($select_kb_res[kb_title]);?></a></td>
                    <td width="15%" align="right">
                      <div style="float:right">
                      <form method="post" action="<?php $_SERVER['PHP_SELF']; ?>" name="delete_kb_form">
                        <input type="hidden" name="kb_id" value="<?=$select_kb_res[id];?>"  />
                        <input type="submit" name="delete_kb" value="Delete" class="ubutton" onclick="return confirm('Are you sure want to delete?');" />
                      </form></div>
                      
                      <div style="float:right"><form method="get" action="kb_sub_cats_posts_edit.php" name="edit_emp">
                        <input type="hidden" name="dept_id" value="<?=$dept_id;?>"  /> 
                        <input type="hidden" name="dept_sub_id" value="<?=$dept_sub_id;?>"  /> 
                        <input type="hidden" name="dept_gdc" value="GuestDelightActivities"  />
                        <input type="hidden" name="kb_id" value="<?=$select_kb_res[id];?>"  /> 
                        <input type="hidden" name="burl" value="<?=$_SERVER['SCRIPT_NAME'];?>"  />                 
