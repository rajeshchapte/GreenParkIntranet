...
$kb_id = $_GET[kb_id];
$sel_kb = mysql_query("SELECT * FROM knwledge_bank WHERE id = '$kb_id'");
$get_kb = mysql_fetch_array($sel_kb);
echo $burl;
...
if(isset($_REQUEST[upload_polc])){
	...
	$kbs = mysql_query("UPDATE  `knwledge_bank` SET  `dept_id` =  '$kb_dept',
			`dept_gdc` =  '$kb_dept_gdc',
			`kb_title` =  '$kb_title',
			`dept_gdc_desc` =  '$kb_text1' WHERE `id` ='$kb_id';");
			
		echo "UPDATE  `knwledge_bank` SET  `dept_id` =  '$kb_dept',
			`dept_gdc` =  '$kb_dept_gdc',
			`kb_title` =  '$kb_title',
			`dept_gdc_desc` =  '$kb_text1' WHERE `id` ='$kb_id';";
	...
	$kbs = mysql_query("UPDATE  `knwledge_bank` SET  `dept_id` =  '$kb_dept',
			`dept_sub_cat_id` =  '$kb_dept_sub_cat',
			`dept_gdc` =  '$kb_dept_gdc',
			`kb_title` =  '$kb_title',
			`dept_gdc_desc` =  '$kb_text1' WHERE `id` ='$kb_id';");
            ...
            <tr>
              ...
              <h2>Create Knowledge Bank</h2>
                ...
                <table width="100%" border="0" cellspacing="5" cellpadding="0">
                  <tr>
                    <td width="25%">Knowledge Bank Title</td><td><input type="text" name="kb_title" id="polc_title" class="adm_inptext" value="<?php echo $get_kb[kb_title]; ?>"  /></td>
                  </tr>
                  <tr>
               ...
               <textarea cols="100" id="editor1" name="editor1" rows="10"><?php echo $get_kb[dept_gdc_desc]; ?></textarea> 
